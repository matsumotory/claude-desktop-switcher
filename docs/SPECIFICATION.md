...
### 1.3. 本アプリケーションの真の価値
「CLI環境の切り替え」は既存のツールで十分です。本アプリ（CSW）の唯一無二の価値は、「**これまで分離が困難だったデスクトップアプリのプロファイルを専用データディレクトリで安全に隔離し、さらにCLIのコンテキスト（`CLAUDE_CONFIG_DIR`）をそれに完全に連動させる統合管理システム**」である点にあります。各プロファイルは自分のディレクトリ内にログインを保持するため、OSキーチェーンから秘密を取り出すことなく分離が成立します。単なるCLIスイッチャーとは異なり、GUIとCLIの双方で一貫したプロファイル状態を保持しながら、特定の設定ファイル（`CLAUDE.md` 等）だけを柔軟に共有（Isolate/Share）できる構造を提供します。
