...
├── statusline/               # 利用量表示を有効にした環境の statusline スクリプトと元設定の退避 (§5.A)
├── usage/                    # 環境ごとの利用量スナップショット <環境名>.json (§5.A)
└── profiles/
...
- **環境ごとの利用量表示（statusline 連携）**: サイドバーの各環境行に、その環境でサインインしているアカウントの利用量（Claude の5時間ごとの上限と1週間ごとの上限の使用率）を表示できる。環境ごとの opt-in（詳細画面のトグル）で、既定ではどの環境の設定にも触れない。
  - **呼称**: ゲージのラベルは、サイドバーと詳細画面で同一の対「5時間」「1週間」（英: "5h" / "week"）とする。どちらも期間を名指しする対称な対で、英語は statusline の既定表示と同語彙。説明文では「5時間ごとの上限」「1週間ごとの上限」（英: the 5-hour limit / the weekly limit）と書く。Claude 本体の現行呼称（`/usage` の "Current session" / "Current week"、ヘルプセンターの "five-hour session limit" / "weekly usage limit"）にそれぞれ対応する（公式の日本語呼称は 2026-07 時点で存在を確認できなかったため、本仕様で確定する）。
  - **データ源**: Claude Code が statusline スクリプトの stdin に渡す JSON の `rate_limits`（`five_hour` / `seven_day` の `used_percentage` と `resets_at`。https://code.claude.com/docs/en/statusline ）。Pro/Max プランのみで、セッションの初回応答後に現れ、各窓は独立に欠落しうる。CSW は欠落を正常系として扱い、値が無い環境にはバーを出さない。
  - **仕組み**: 有効化すると、その環境の `cli-data/settings.json` の `statusLine` を CSW が生成するスクリプト（`~/.context-switcher-claude/statusline/<環境名>.sh`）に向ける。スクリプトは受け取った JSON を `~/.context-switcher-claude/usage/<環境名>.json` へ保存し（保存先の環境名は `CLAUDE_CONFIG_DIR` から導出し、導出できないパスでは保存しない）、ステータスラインには「5h 42% / week 13%」形式の簡潔な使用率を表示する。既存の statusline 設定がある環境では元の設定を退避した上で、スクリプトが元のコマンドへ stdin を透過してその出力を表示し続ける（共存）。無効化すると退避した元の設定を復元し、スクリプトと保存データを削除する。環境の削除時も同様に削除する。
  - **有効/無効の判定**: 別のフラグで二重管理せず、settings.json の `statusLine` が CSW のスクリプトを指しているかどうかの実態から導出する。ユーザーが settings.json を手動で変更していた場合、無効化はその設定に触れない。
  - **安全条件**: 既存の Claude（`default`）はトグルで明示的に有効化した場合のみ `~/.claude/settings.json` に設定し、無効化で復元する（ゼロインパクト保証 §6 の「既定では触れない」を維持）。ツール権限・フック（`cli_settings`）を「共有」にしている環境は、settings.json が既存の Claude と同一実体（シンボリックリンク）であり、書き込むと既存の Claude 側も変わるため有効化できない（UI で理由を表示する）。
  - **表示形**: サイドバーの各環境行には、ゲージ 2 本（5時間、1週間）を項目ラベルと使用率の数値付きで表示し、ゲージ単独で置かない。塗り色は残量に応じて変わる（70% 未満 = 緑、70% 以上 = 琥珀、90% 以上 = 赤。共有・分離・削除の意味色とは別のゲージ専用色）。詳細画面には同じラベルの下にリセット時刻を縦積みで添えた同じゲージを表示する（長い日付がバーの幅を奪わない配置）。
  - **鮮度**: 値が更新されるのは Claude Code の応答時のみ（Desktop アプリの利用だけでは更新されない）。GUI は保存ファイルを 60 秒間隔とウィンドウのフォーカス時に読み直し（ローカル read のみ）、取得からの経過時間を添えて表示する。取得から 10 分を超えた値は淡色で示し、`resets_at` を過ぎた窓の値は表示しない（リセット後の実際の使用率は不明であり、古い値を新鮮に見せない）。
  - **プライバシー**: この機能もインターネット通信は行わず、認証情報ストアには触れない。CSW が書き込むのは、明示的に有効化した環境の settings.json（statusLine キー）と CSW 自身のデータディレクトリだけで、利用量の数値は Claude Code 自身がローカルに渡してくるものを保存・表示する。
