...
- `desktop_app_config`: Desktopアプリ設定 (`config.json`)
...
- CLI用: Service: `Claude Code-credentials`, Account: ログインユーザー名（macOS のユーザー名）

> 既知の課題: 現行コードは CLI 用 Account を `CloudFlare` と固定しているが、実際の Claude Code はログインユーザー名を Account に用いるため、現状の退避処理は機能していない。なお実機検証の結果、Claude Code の認証は `CLAUDE_CONFIG_DIR` 単位でネイティブに分離されるため、そもそもキーチェーン退避処理は不要であり、撤去を予定している（下記参照）。

**スイッチング・フロー (`switcher.switch_to`)**:
1. 現在のプロファイルのキーチェーン情報を JSON (`cli-data/keychain_backup.json`) として退避。

> 既知の課題（セキュリティ）: 現行の退避ファイルは**暗号化されておらず平文**で、ファイル権限のハードニングも未実装。秘密情報をディスクへ平文で持ち出すのはキーチェーン採用の意義を損なう。実機検証の結果、デスクトップは `--user-data-dir`、CLI は `CLAUDE_CONFIG_DIR` それぞれのディレクトリ単位でセッション/認証がネイティブに分離されることが判明したため、**このキーチェーン退避処理自体を撤去**し、ディレクトリ隔離のみで分離する方針へ移行する。
...
- 初回起動時には 3 ステップのオンボーディング（ようこそ／使い方／ターミナル連携）が表示される（`localStorage` の `csw_onboarded` で初回判定）。
