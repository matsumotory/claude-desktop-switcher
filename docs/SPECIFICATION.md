...
- **表示言語（i18n）**: ユーザー可視の表示は日本語と英語の2言語。システムロケールが `ja` で始まるときのみ日本語、それ以外はすべて英語で表示する（判定できないときも英語）。この規則は設定ウィンドウ（WebView。`?lang=ja|en` クエリで上書き可能、スクリーンショット生成用）と、WebView を経由しないネイティブ表示（システムトレイのメニュー）の両方に適用する。トレイの英語ラベルは WebView と同じ確定語彙を使う（「既存の Claude」= "Existing Claude"、「(利用中)」= "(In use)"、「設定...」= "Settings..."、「終了」= "Quit"）。ユーザーデータ（ユーザーが付けた環境名）は、どの言語でも翻訳しない。属性経由の文言（`aria-label`・`placeholder`・`title`）も可視文字列と同様に対象言語で表示する。
...
- `csw profile list | show <name> | delete <name>`: 環境（プロファイル）の一覧表示・詳細表示・削除
