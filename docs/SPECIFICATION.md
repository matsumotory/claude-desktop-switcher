...
### B. CLI コマンド (`csw`)
> 配布についての注記: `csw`（CLI）は主にエンジニアが使う想定のため `.dmg` には同梱しない（DMG はメニューバー GUI のみ）。CLI を使うときはソースからビルドして導入する（`cargo install --path crates/cli` で `~/.cargo/bin/csw` に入る）。GUI からの環境切り替えだけなら不要。
