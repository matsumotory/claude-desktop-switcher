...
### B. CLI コマンド (`csw`)
> 配布についての注記: `csw` バイナリは現状 `.dmg` に同梱しておらず（DMG はメニューバー GUI のみ）、CLI を使うにはソースからビルドして導入する（`cargo install --path crates/cli` で `~/.cargo/bin/csw` に入る）。GUI からの環境切り替えだけなら不要。`.app` への同梱と PATH 導入操作の提供は今後の課題。
