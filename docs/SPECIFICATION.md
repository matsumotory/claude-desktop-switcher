...
- `settings` メニューから環境の操作（Tauri commands: `create_profile`, `clone_profile`（複製）, `switch_profile`, `delete_profile`（ゴミ箱へ移動）, `purge_profile`（完全削除）, `launch_additional_window`（完全分離環境を追加のウィンドウで起動）, `inspect_profile`（分離の検査）, `get_profile_data_map`（データの内訳）, `reveal_profile_dir`（Finder で表示）, `get_profile_details`, `list_profiles`, `get_active_profile`, `get_desktop_running_status`, `get_running_profiles`（起動中の環境名の一覧）, `get_default_roots_status`, `app_version`, `open_url`, `get_dmg_mount_status`（マウント中の CSW ディスクイメージの検出）, `eject_dmg`（検出済みマウントポイントの取り出し））を提供するWebUIを表示可能。`open_url` は固定の https GitHub URL のみを既定ブラウザで開く（アプリ自体は通信しない）。
...
- `csw profile list | show <name> | delete <name> [--purge]`: 環境（プロファイル）の一覧表示・詳細表示・削除。削除は既定で、環境のフォルダをそのまま macOS のゴミ箱へ移動する（共有リンクはリンクのまま一緒に移動し、既存の Claude 側は無傷）。ゴミ箱を空にするまでは、フォルダを `profiles/` へ戻せば、サインイン状態や共有リンクを含めて完全に復元される（一覧への再表示はディレクトリ走査で自動）。`--purge` はゴミ箱を経由せず完全削除する。ゴミ箱への移動に失敗したときは、完全削除へ切り替えずエラーとして伝え、`--purge` を案内する
