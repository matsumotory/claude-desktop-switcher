...
└── profiles/
    └── <Profile_Name>/       # 各環境の専用ディレクトリ
        ...
        ├── state.toml        # 起動状態 (CSW からの最終起動日時)。起動のたびに profile.toml を書き換えないための分離
...
- **環境のメモと来歴**: 各環境は自由記述のメモを持てる。メモは 1 行 200 文字までで、作成ダイアログで入力でき、環境詳細画面の「この環境のメモ」でいつでも編集できる（Tauri command: `set_profile_note`。空文字でクリア）。メモはユーザーデータであり、翻訳もパースもしない。環境一覧の各行にはメモと「最終起動」の相対時刻をサブテキストで表示し、詳細画面には作成日時・複製元・最終起動を表示する。最終起動は、「切り替えて起動」「重複して起動」や `csw switch` のような、起動を伴う操作が成功したときに `state.toml` へ temp+rename で記録する。安全宣言である `profile.toml` を起動のたびに書き換えないための分離である。`csw switch --no-launch` では記録しない。既存の Claude（`default`）はメモ・来歴・最終起動の対象外で、実データ側に `state.toml` を作らない。複製はメモを複製元から引き継ぐ。
