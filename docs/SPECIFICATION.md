...
- **環境ごとの利用量表示（statusline 連携）**: サイドバーの各環境行に、その環境でサインインしているアカウントの利用量（Claude のセッション上限と週間上限の使用率）を表示できる。環境ごとの opt-in（詳細画面のトグル）で、既定ではどの環境の設定にも触れない。
  - **呼称**: Claude 本体の現行呼称（`/usage` の "Current session" / "Current week"、ヘルプセンターの "five-hour session limit" / "weekly usage limit"）に追従し、英語は "Session" / "Week"、日本語は「セッション（5時間）」「週間」とする（公式の日本語呼称は 2026-07 時点で存在を確認できなかったため、本仕様で確定する）。
  ...
  - **表示形**: サイドバーの各環境行には、ゲージ 2 本（セッション、週間）を項目ラベル（5h ／ 週）と使用率の数値付きで表示し、ゲージ単独で置かない。塗り色は残量に応じて変わる（70% 未満 = 緑、70% 以上 = 琥珀、90% 以上 = 赤。共有・分離・削除の意味色とは別のゲージ専用色）。詳細画面には正式名（「セッション（5時間）」「週間」）とリセット時刻を添えた同じゲージを表示する。
