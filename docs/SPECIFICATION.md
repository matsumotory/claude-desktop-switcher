...
- **環境ごとの利用量表示（statusline 連携）**: サイドバーの各環境行に、その環境でサインインしているアカウントの利用量（Claude のセッション上限と週間上限の使用率）を表示できる。環境ごとの opt-in（詳細画面のトグル）で、既定ではどの環境の設定にも触れない。
  ...
  - **鮮度**: 値が更新されるのは Claude Code の応答時のみ（Desktop アプリの利用だけでは更新されない）。GUI は保存ファイルを 60 秒間隔とウィンドウのフォーカス時に読み直し（ローカル read のみ）、取得からの経過時間を添えて表示する。`resets_at` を過ぎた窓の値は表示しない（リセット後の実際の使用率は不明であり、古い値を新鮮に見せない）。
