...
### 環境ごとの利用量を表示する（Pro / Max プラン向け）

サイドバーの各環境の行に、Claude の5時間ごとの上限と1週間ごとの上限の使用率を表示できます。どのアカウントにどれだけ余裕があるかを、環境を切り替える前に一目で確認できます。

* **有効化**: 環境の詳細画面にある「利用量」のスイッチをオンにします。環境ごとのオプトインで、オンにした環境の Claude Code の設定（settings.json）に、利用量を記録するステータスライン設定が追加されます。すでに独自のステータスラインを設定している場合は、その表示を保ったまま記録だけが加わり、オフにすると元の設定に戻ります。
* **見かた**: 各バーには項目（5時間 ／ 1週間）と使用率の数値が付き、バーの色は残量に応じて変わります（余裕があるうちは緑、70% 以上で琥珀、90% 以上で赤）。
* **値の更新タイミング**: 値が更新されるのは、その環境のターミナルで Claude Code が応答したときだけです。デスクトップアプリの利用だけでは更新されません。古い値は取得からの経過時間を添えた淡い表示になります。
* **表示される条件**: 利用量の値は、Pro または Max プランのサブスクリプションでサインインしている場合に Claude Code から渡されます。API キーでの利用では表示されません。
* **通信しません**: この機能もインターネット通信は行いません。Claude Code が手元に書き出す値を読んで表示するだけです。
...
**Q. 各環境の利用量（レート制限の使用率）を確認できますか？**
できます。環境の詳細画面で「利用量」をオンにすると、サイドバーのその環境の行に、5時間ごとの上限と1週間ごとの上限の使用率バーが表示されます。値はその環境のターミナルで Claude Code が応答したときに更新され、インターネット通信は行いません。Pro または Max プランでのサインインが必要です。詳しくは「2. 日々の使い方」の「環境ごとの利用量を表示する」を参照してください。
