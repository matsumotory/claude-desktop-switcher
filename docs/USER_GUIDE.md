...
* **間違えて環境を消しても戻せます**
  環境の削除は、フォルダごと macOS のゴミ箱へ移動するだけです。ゴミ箱を空にするまでは、サインイン状態や共有リンクも含めて何も失われていません。戻したいときは、ゴミ箱の中のフォルダを `~/.context-switcher-claude/profiles/` へ移動してください。CSW のウィンドウに戻ると、一覧にそのまま再表示されます。すぐに消したいときは、削除の確認で「完全に削除」を選びます（ゴミ箱を経由せず、戻せません）。
* **この環境のデータはどこに何があるかを見通せます**
  環境の詳細画面の「場所（この環境のデータ）」を開くと、データフォルダのパスに加えて、この環境が専有している容量と、フォルダごとの概算サイズ、項目ごとの内訳が表示されます。共有している項目はリンク先を、それ以外は概算サイズと最終更新を示します。表示のために読むのはファイル名とサイズ・最終更新の日時だけで、中身は開きません。各フォルダは「Finder で表示」でそのまま開けるので、バックアップのときも迷いません。
