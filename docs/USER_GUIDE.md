...
サイドバーの各環境の行に、Claude のセッション上限（5時間）と週間上限の使用率を表示できます。どのアカウントにどれだけ余裕があるかを、環境を切り替える前に一目で確認できます。
...
* **見かた**: 各バーには項目（5h ／ 週）と使用率の数値が付き、バーの色は残量に応じて変わります（余裕があるうちは緑、70% 以上で琥珀、90% 以上で赤）。
...
できます。環境の詳細画面で「利用量」をオンにすると、サイドバーのその環境の行に、セッション上限（5時間）と週間上限の使用率バーが表示されます。値はその環境のターミナルで Claude Code が応答したときに更新され、インターネット通信は行いません。Pro または Max プランでのサインインが必要です。詳しくは「2. 日々の使い方」の「環境ごとの利用量を表示する」を参照してください。
