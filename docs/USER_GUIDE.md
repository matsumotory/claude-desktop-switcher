...
3. アプリを起動すると設定ウインドウが開きます。あわせて画面右上のメニューバーにアイコンが表示されます（メニューバーが混み合っていると隠れることがありますが、Dock アイコンからいつでも設定ウインドウを開けます）。
...
> **メモ: 環境の同時起動**
> 設定を共有する環境（「会話とメモリも分ける」「アカウントだけ分ける」）は、設定の衝突を防ぐため一度に1つずつ開きます。個人用に戻すときは、起動中の Claude を終了し、サイドバーの「既存の Claude」を選んで「既存の Claude に切り替える」を押してから、いつもどおり Claude を起動してください。一方、「すべて分ける」で作った環境は何も共有しないため、起動中の Claude を終了せずに、詳細画面の「新しいウィンドウで起動」で並べて同時に開けます。
...
CSW から開いたアプリの中で使うターミナル（内蔵）は、最初からそのアプリと同じ環境です。コマンドは不要です。一方、ご自身で別に開いたターミナル（外部）は普段の環境のままなので、`eval $(csw env Work)` を実行して明示的に揃えてください（そのタブだけに適用され、普段の環境には影響しません）。なお、環境（設定ディレクトリ）は自動で揃いますが、その環境で初めて Claude Code を使うときは CLI のサインインが別途一度だけ必要です（次の Q を参照）。
