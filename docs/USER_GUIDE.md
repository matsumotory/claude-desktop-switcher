...
このコマンドには CLI ツール `csw` が必要です。`csw` は現在 `.dmg` に同梱されていないため、ターミナル連携を使う場合のみソースから導入してください（本リポジトリで `cargo install --path crates/cli` を実行すると `~/.cargo/bin/csw` に入ります。Rust の開発環境が必要です）。デスクトップアプリだけで使う場合は不要です。
