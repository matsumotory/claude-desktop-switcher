...
- ユーザーフィードバックにより、ゲージは項目ラベル（5h ／ 週）と数値を常時表示し、残量を色（70% 未満 = 緑、70%+ = 琥珀、90%+ = 赤）で示す形に強化した。
- CI の macOS runner で、plutil がキー欠落時のエラーを stdout に出す個体差により偽の 0% 表示が起きることを検出し、数値ガードで修正（1ccb779）。

## 撤去（2026-07-02）

本 plan で実装した statusline 連携は撤去した（後継の判断記録は [usage-display-removal.md](usage-display-removal.md)）。理由の要約: statusline はデスクトップアプリ単独の利用時に更新されず、CSW の主対象であるデスクトップ利用者には値が出ない。デスクトップ分を含む使用率を取る手段は、いずれも CSW の原則（通信しない・認証情報に触れない・他アプリのローカルデータに触れない）と両立しないことをサーベイで確認したため、機能ごと取り下げた。この plan は「一度出荷したが原則を守れないと分かって撤去した」記録として `rejected` で残す。
